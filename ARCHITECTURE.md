...
- Reusable runtime lives in `lib/dom_ui/` (components, hooks, DOM DSL, routing helpers).
...
- `lib/dom_ui/component.dart`: `Component` base class, scheduler, hooks (`useEffect`, `useRef`, `useMemo`, `useReducer`), context (`provide`/`useContext`)
- `lib/dom_ui/dom.dart`: small DOM builder DSL
